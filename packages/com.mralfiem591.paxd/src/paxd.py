...
def cleanup():
    ...
    if needs_rewrite:
        ...
        with open(bat_file_path, 'w') as f:
            f.write("@echo off\n")
            f.write(sys.executable + f"{CURRENT_FILE_PATH} %*\n")
        ...
        try:
            ...
            if mainfile:
                ...
                if not os.path.exists(bat_file_path):
                    ...
                    with open(bat_file_path, 'w') as f:
                        f.write(f"@echo off\n")
                        f.write(sys.executable + f"{os.path.join(local_app_data, 'com.mralfiem591.paxd', 'run_pkg.py')} {os.path.join(local_app_data, package_name, mainfile)} %*\n")
    ...
    if os.path.exists(os.path.join(os.path.dirname(__file__), ".FIRSTRUN")):
        ...
        if not os.path.exists(paxd_bin_path):
            with open(paxd_bin_path, 'w') as f:
                f.write(f"@echo off\n")
                f.write(sys.executable + f"{os.path.abspath(__file__)} %*\n")
